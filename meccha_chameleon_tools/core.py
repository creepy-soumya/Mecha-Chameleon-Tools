...
class MecchaESP:
    ...
    def get_actor_root_pos(self, actor):
        ...
        if not root:
            return None
        # FBoxSphereBounds.Origin is FVector (3 x double in UE5 = 24 bytes) at offset 0x140.
        pos = rvec3(self.pm, root + 0x140)
        if pos[0] == 0.0 and pos[1] == 0.0 and pos[2] == 0.0:
            # Fallback to RelativeLocation for pawns with uninitialized bounds
    ...
    def iter_players(self, include_local=True, team_filter=False, enemy_only=False):
        ...
        for i in range(pa_count):
            ...
            # - Only skip if health is definitively 0 AND the role is known (not Unknown).
